/* These settings can be customized by the user need */

/*
 This is the link to your domain...it redirects the user to your myaccount page.
 It is used on the site on buttons like: Buy now, Order now, Members, Buy HTTP proxies, etc.
*/
export const MEMBERS_URL = "https://my.proxy4less.com";

export const SIGNUP_URL = "https://my.proxy4less.com/signup";

/*
 Your site name - this appears in the footer
*/
export const SITE_NAME = "PROXY4LESS";

/*
 The name of you site that appears in Terms of service section - it can be you domain, your company name, etc.
*/
export const TOS_SITE_NAME = 'PROXY4LESS';

/*
 The date when the terms of service where last updated.
*/
export const TOS_LAST_UPDATE = 'Apr-11-2020';

/*
 This is the link of your chat widget - it can be found on your tawk to account
*/
export const TAWK_TO_WIDGET_URL = '';

/*
 This is the link to your backend services. It is used to get the data for prices and datacenters. Normally, you shouldn't touch this one
*/
export const BACKEND_URL = 'https://backend.changemyip.com';

/*
 Meta tags for SEO - description and keywords for this site
*/
export const SEO_DESCRIPTION = 'Buy clean residential and datacenter private proxies with Bitcoin, PayPal, PerfectMoney. US residential proxies are available to buy for affordable price. Shared proxies starting at $0.99 per month.';
// The keywords need to be separated by comma
export const SEO_KEYWORDS = 'anonymous private shared proxy, dedicated proxy, datacenter proxy, socks, shared proxies, dedicated proxies, residential proxies, socksv5, us residential proxy, europe residential proxy';

